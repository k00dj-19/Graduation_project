...
class Transformer(nn.Module):
    ...
    def forward(self, src, mask, query_embed, pos_embed, saliency_proj1, video_length=None): # 和主函数权重共享
        ...
        #print("query_embed", query_embed.shape) # 10, 2
        src_txt = src[video_length:]
        #print("src_txt", src_txt.shape) # L_txt, batch, d
        refpoint_embed = query_embed.unsqueeze(1).repeat(1, bs, 1)  # (#queries, batch_size, d)
        #print("refpoint_embed", refpoint_embed.shape) # 10, b, 2
        ...
        #tgt = src_txt.max(dim=0).values.unsqueeze(0).repeat(refpoint_embed.shape[0], 1, 1)
        #tgt = src_txt.mean(dim=0).unsqueeze(0).repeat(refpoint_embed.shape[0], 1, 1)
        #print('tgt shape:', tgt.shape) # 10, 32, 256
        ...
        #print("hs", hs.shape) # 2, batch, #queries, 256
        #print("references", references.shape) # 2, batch, #queries, 2
