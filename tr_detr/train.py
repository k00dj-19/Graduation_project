...
def train_epoch(model, criterion, train_loader, optimizer, opt, epoch_i, tb_writer):
    ...
    for batch_idx, batch in tqdm(enumerate(train_loader),
        ...
        #print('src_cap_mask:', model_inputs['src_cap_mask'].shape)
        src_cap_mask = model_inputs['src_cap_mask'].max(dim=-1).values # batch, 75
        pos_mask =  targets['src_pos_mask'] 

        src_txt_ed, src_vid_ed =  outputs['src_txt_ed'], outputs['src_vid_ed']
        src_cap_ed = outputs['src_cap_ed']
        loss_align = CTC_Loss()
        loss_align2 = CTC_Loss()
        loss_vid_txt_align = loss_align(src_vid_ed, src_txt_ed, pos_mask, src_vid_mask, src_txt_mask )
        loss_cap_txt_align = loss_align2(src_cap_ed, src_txt_ed, pos_mask, src_cap_mask, src_txt_mask )
        src_vid_cls_ed = outputs['src_vid_cls_ed']
        src_txt_cls_ed = outputs['src_txt_cls_ed']
        #src_cap_cls_ed = outputs['src_cap_cls_ed']
        loss_align_VTC = VTCLoss()
        #loss_align_VTC2 = VTCLoss()
        loss_vid_txt_align_VTC = loss_align_VTC(src_txt_cls_ed, src_vid_cls_ed)
        #loss_cap_txt_align_VTC = loss_align_VTC2(src_txt_cls_ed, src_cap_cls_ed)
        
        losses = losses  + opt.VTC_loss_coef*loss_vid_txt_align_VTC + opt.CTC_loss_coef*loss_vid_txt_align + opt.CTC_loss_coef*loss_cap_txt_align
