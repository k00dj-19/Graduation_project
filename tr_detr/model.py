...
class CrossModalEnhancement(nn.Module):
    def __init__(self, d_model, n_heads):
        super(CrossModalEnhancement, self).__init__()
        self.video_to_caption = CrossAttention(d_model, n_heads)  # Video -> Caption
        self.caption_to_video = CrossAttention(d_model, n_heads)  # Caption -> Video

    def forward(self, video_features, caption_features):
      
        # Cross-Attention Forward Pass
        enhanced_caption = self.video_to_caption(caption_features, video_features, video_features)
        enhanced_video = self.caption_to_video(video_features, caption_features, caption_features)

        return enhanced_video, enhanced_caption
...
class TRDETR(nn.Module):
    ...
    def __init__(self, transformer, position_embed, txt_position_embed, txt_dim, vid_dim, cap_dim,
        ...
        #self.cross_modal_enhance = CrossModalEnhancement(transformer.d_model, 8)
    ...
    def forward(self, src_txt, src_txt_mask, src_vid, src_vid_mask, src_cap, src_cap_mask, src_aud=None, src_aud_mask=None):
        ...
        #print("src_vid shape: ",src_vid.shape)
        #print("src_cap shape", src_cap.shape)
        src_vid = self.input_vid_proj(src_vid) # (bsz, L_vid, d=256)
        src_txt = self.input_txt_proj(src_txt)  # (bsz, L_txt, d=256)
        src_cap = self.input_cap_proj(src_cap)  # (bsz, L_vid, L_cap, d=256)
      
        refined_src_cap = self.attention_weighted_sum(src_cap, src_txt)  # (bsz, L_vid, d=256)
        
        # CTC
        src_txt_ed = src_txt
        src_vid_ed = src_vid
        src_cap_ed = refined_src_cap
        # VTC
        src_vid_cls_ed = src_vid.mean(1)
        src_txt_cls_ed = src_txt.mean(1)
        src_cap_cls_ed = refined_src_cap.mean(1)

        src_vid = self.fuser1(src_vid, src_txt, src_vid_mask, src_txt_mask)  # (bsz, L_vid, d)
        src_cap = self.fuser2(refined_src_cap, src_txt, src_vid_mask, src_txt_mask)  # (bsz, L_vid, d)

        
        # Forward pass
        #enhanced_vid, enhanced_cap = self.cross_modal_enhance(src_vid, refined_src_cap)

        # concat vid and caption
        src_vid = torch.cat([src_vid, src_cap], dim=2)  # (bsz, L_vid, d*2)
        src_vid = self.weighted_vid_cap_proj(src_vid) # (bsz, L_vid, d)

        src_vid = self.fuser(src_vid, src_txt, src_vid_mask, src_txt_mask)  # (bsz, L_vid, d)
        ...
        outputs_class = self.class_embed(hs)  # (#layers, batch_size, #queries, #classquery_embedes)
        ...
        out['src_cap_cls_ed'] = src_cap_cls_ed
    ...
    def attention_weighted_sum(self, caption_feature, text_query_feature):
        # caption_feature: [batch_size, 75, 32, 256]
        # text_query_feature: [batch_size, txt, 256]

        # 1. 유사도 계산 (각 배치의 프레임 32개의 캡션 시퀀스와 텍스트 쿼리 간의 유사도 계산)
        # text_query_feature의 shape을 맞추기 위해 [batch_size, 1, txt, 256]로 변환
        text_query_feature = text_query_feature.unsqueeze(1)  # [batch_size, 1, txt, 256]
       
        # caption_feature와 텍스트 쿼리 간의 내적(dot product) 유사도 계산
        # 유사도: [batch_size, 75, 32, 256] · [batch_size, 1, txt, 256].transpose(-1, -2) -> [batch_size, 75, 32, txt]
        similarity = torch.matmul(caption_feature, text_query_feature.transpose(-1, -2))  # [batch_size, 75, 32, txt]
      
        # 2. Softmax를 통해 가중치 계산 (각 프레임의 32개의 시퀀스에 대한 가중치)
        # txt 차원에 대해 평균을 내어 [batch_size, 75, 32] 크기의 가중치 계산
        attention_weights = F.softmax(similarity.mean(dim=3), dim=2)  # [batch_size, 75, 32]

        # 3. 가중치를 사용하여 가중합 계산
        # 가중치를 [batch_size, 75, 32, 1]로 변환하여 caption_feature와 곱함
        attention_weights = attention_weights.unsqueeze(3)  # [batch_size, 75, 32, 1]
     
        # 가중합 계산 (Weighted Sum), [batch_size, 75, 32, 256] * [batch_size, 75, 32, 1] -> [batch_size, 75, 32, 256]
        weighted_caption = caption_feature * attention_weights  
      
        # 시퀀스 차원(32)에 대해 합산하여 [batch_size, 75, 256]로 변환
        caption_feature_pooled = weighted_caption.sum(dim=2)  # [batch_size, 75, 256]
